...
# -------------------------------------------------------
def population_init(REDGE, ED, EC, SHORTEST_DIS, num, CAPACITY, DEPOT, seed,end,POPULATION_SIZE):
    population = set()
    # random.seed(seed)
    while len(population) < num:
        ...
        if random.random() > sol.discard_prop:
            population.add(sol)
    best=searching(population, REDGE, ED, EC, SHORTEST_DIS, CAPACITY, DEPOT, seed, POPULATION_SIZE, end)
    return best
...
def MS(solution, REDGE, ED, EC, SHORTEST_DIS, CAPACITY, DEPOT):
    ...
    while (True):
        ...
        verhicles1=verhicles
        # if verhicles>5:
        #     if random.random() > 0.3:
        #         verhicles = int(verhicles / 3)
        if verhicles1>3:
            if random.random() > 0.1:
                verhicles=int(verhicles1/2)
        # if verhicles1>5:
    ...
    for i in range(verhicles):
        ...
        new_solution.loads[vehicle[i]] = tempt_solution.loads[i]
...
def ini_ps(end):
    ...
    for i in range(CPU):
        result.append((pool.apply_async(population_init, args=(
            REDGE, ED, EC, SHORTEST_DIS, num, CAPACITY, DEPOT, SEED + i,end,population_size,))))
    pool.close()
    pool.join()
    bests = []
    for i in result:
